...
// ─── Extended profile shape from /me/complete ─────────────────────────────────
// The complete endpoint resolves profilePictureId → a full URL.
...
export const MainHeader: React.FC = () => {
  ...
  // ── Profile hook ───────────────────────────────────────────────────────
  // Only auto-fetch when the user is authenticated — mirrors the original
  // `autoLoad: isAuthenticated` behaviour.
  const {
    profile,
  ...
  // The complete endpoint enriches the base profile with a resolved picture URL.
  ...
  const [latestProfilePictureUrl, setLatestProfilePictureUrl] = useState<
    string | undefined
  >(extendedProfile?.profilePictureUrl);

  // Sync picture URL whenever the complete profile resolves.
  React.useEffect(() => {
    if (extendedProfile?.profilePictureUrl) {
      setLatestProfilePictureUrl(extendedProfile.profilePictureUrl);
    }
  }, [extendedProfile?.profilePictureUrl]);

  const isLoading = authLoading || (isAuthenticated && profileLoading.profile);
  ...
  const handleProfilePictureUpdate = async (url: string) => {
    // Immediately reflect the new URL, then re-fetch the enriched profile.
    setLatestProfilePictureUrl(url);
    try {
      await fetchCompleteProfile();
    } catch {
      // non-critical — UI already shows the optimistic URL
    }
  };
